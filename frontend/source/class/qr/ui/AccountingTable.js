...
qx.Class.define("qr.ui.AccountingTable", {
    ...
    construct: function(){
        ...
        var balance = new qx.ui.basic.Atom(this.tr('Balance: %1 CHF',0)).set({center: true}).set({
            font: 'headline'
        });
        tb._add(balance);
        var model = new qr.data.RemoteTableModel('accounting');
        model.setColumns(
            [this.tr('AcctId'),this.tr('Date'),this.tr('Remark'),this.tr('User'),this.tr('ResId'),this.tr('Amount')],
